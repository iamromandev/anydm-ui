...
const vnode_getChildWithIdx = (vNode, childIdx) => {
    ...
    for (assertDefined(child, "Missing child."); child.flags >>> 8 !== childIdx; ) {
...
const vnode_getVNodeForChildNode = (vNode, childElement) => {
    ...
    let child = vnode_getFirstChild(vNode);
    for (assertDefined(child, "Missing child."); child && (!(child instanceof ElementVNode) || child.element !== childElement); ) {
    ...
    return ensureElementVNode(child), assertEqual(child.element, childElement, "Child not found."), 
    child;
};
    ...
    for (;idx < length; ) {
        switch (journal[idx++]) {
          ...
          case 2:
            ...
            isBooleanAttr(element, key) ? element[key] = parseBoolean(value) : "value" === key && key in element ? element.value = String(value) : key === dangerouslySetInnerHTML ? (element.innerHTML = value, 
...
const vnode_insertBefore = (journal, parent, newChild, insertBefore) => {
    ...
    if (!parentIsDeleted) {
        ...
        domChildren && domChildren.length && journal.push(6, parentNode, vnode_getNode(adjustedInsertBefore), ...domChildren);
    }
...
const vnode_getElementName = vnode => {
    ...
    if (void 0 === elementName) {
        ...
        elementName = elementVNode.elementName = nodeName, elementVNode.flags |= vnode_getElementNamespaceFlags(element);
    }
    return elementName;
};
...
const vnode_setText = (journal, textVNode, text) => {
    ...
    journal.push(1, textVNode.textNode, textVNode.text = text);
};
...
const ensureMaterialized = vnode => {
    ...
    if (void 0 === vFirstChild) {
        vFirstChild = vParent.parent && shouldIgnoreChildren(vParent.element) ? vParent.firstChild = vParent.lastChild = null : vnode_materialize(vParent);
    }
...
const hasQStyleAttribute = element => "STYLE" === element.nodeName && (element.hasAttribute("q:sstyle") || element.hasAttribute(QStyle));
    ...
    if (vParent.lastChild = vChild || null, vParent.firstChild = vFirstChild, vData) {
        ...
        processVNodeData$1(vData, (peek, consumeValue) => {
            if (peek() === VNodeDataChar.ID) {
                ...
                container.$setRawState$(parseInt(id), vParent), isDev && vParent.setAttr("q:id", id, null);
            } else {
                peek() === VNodeDataChar.BACK_REFS ? (container || (container = getDomContainer(vParent.element)), 
...
const vnode_getNode = vnode => null === vnode || vnode_isVirtualVNode(vnode) ? null : vnode_isElementVNode(vnode) ? vnode.element : (assertTrue(vnode_isTextVNode(vnode), "Expecting Text Node."), 
vnode.textNode);
...
const newInvokeContextFromTuple = ([element, event, url]) => {
    ...
    return locale && setLocale(locale), newInvokeContext(locale, hostElement, element, event, url);
};
...
class JSXNodeImpl {
    ...
    constructor(type, varProps, constProps, children, key, toSort, dev) {
        ...
        this.children = children, "string" == typeof type) {
            ...
            BIND_CHECKED in this.varProps ? toSort = handleBindProp(this.varProps, BIND_CHECKED) || toSort : BIND_VALUE in this.varProps ? toSort = handleBindProp(this.varProps, BIND_VALUE) || toSort : this.constProps && (BIND_CHECKED in this.constProps ? handleBindProp(this.constProps, BIND_CHECKED) : BIND_VALUE in this.constProps && handleBindProp(this.constProps, BIND_VALUE)), 
    ...
    return maybeThen(jsxElement, jsxElement => {
        ...
        for (const key in useOnEvents) {
            if (Object.prototype.hasOwnProperty.call(useOnEvents, key)) {
                ...
                let eventKey = key;
                if (isHeadless) {
                    if ("on:qvisible" !== key && !key.startsWith("on-document:") && !key.startsWith("on-window:")) {
                ...
                targetElement && ("script" === targetElement.type && "on:qvisible" === key && (eventKey = "on-document:qinit", 
...
const vnode_diff = (container, jsxNode, vStartNode, scopedStyleIdPrefix) => {
    ...
    function descend(children, descendVNode, shouldExpectNoChildren = !0) {
        ...
        }() : (stackPush(children, descendVNode), descendVNode && (assertDefined(vCurrent || vNewNode, "Expecting vCurrent to be defined."), 
        vSideBuffer = null, vSiblings = null, vSiblingsArray = null, vParent = vNewNode || vCurrent, 
        vCurrent = vnode_getFirstChild(vParent), vNewNode = null), shouldAdvance = !1);
    }
    function ascend() {
    ...
    function expectProjection() {
        ...
        isDev && vNewNode.setProp("q:type", "P"), isDev && vNewNode.setProp("q:code", "expectProjection"), 
    ...
    function expectSlot() {
        ...
        isDev && vNewNode.setProp("q:type", "P"), isDev && vNewNode.setProp("q:code", "expectSlot" + count++), 
    ...
    function expectElement(jsx, elementName) {
        ...
        }(vNode, jsxAttrs, isDev && getFileLocationFromJsx(jsx.dev) || null) || needsQDispatchEventPatch, 
    ...
    function expectVirtual(type, jsxKey) {
        ...
        const createNew = () => {
            ...
            vNewNode.setProp("q:key", jsxKey), isDev && vNewNode.setProp("q:type", type);
        };
    ...
    function insertNewComponent(host, componentQRL, jsxProps) {
        ...
        const jsxNode = jsxValue;
        isDev && vNewNode.setProp("q:type", "C"), container.setHostProp(vNewNode, "q:renderFn", componentQRL), 
    ...
    function insertNewInlineComponent() {
        vnode_insertBefore(journal, vParent, vNewNode = vnode_newVirtual(), vCurrent && getInsertBefore());
        const jsxNode = jsxValue;
        isDev && vNewNode.setProp("q:type", "I"), vNewNode.setProp("q:props", jsxNode.props), 
...
function cleanupStaleUnclaimedProjection(journal, projection) {
    ...
    if (projectionParent) {
        1 & projectionParent.flags && vnode_getElementName(projectionParent) === QTemplate && vnode_remove(journal, projectionParent, projection, !0);
    }
}
...
function isDescendant(descendantChore, ancestorChore) {
    ...
    return !(!vnode_isVNode(descendantHost) || !vnode_isVNode(ancestorHost)) && vnode_isDescendantOf(descendantHost, ancestorHost);
}
...
function debugTrace(action, arg, queue, blockedChores) {
    ...
    if (queue && queue.length > 0) {
        lines.push(""), lines.push(`📋 Queue (${queue.length} items):`);
...
const _fnSignal = (fn, args, fnStr) => new WrappedSignalImpl(null, fn, args, fnStr || null);
...
function processJSXNode(ssr, enqueue, value, options) {
    if (null == value) {
        ...
        if (Array.isArray(value)) {
            ...
            ssr.openFragment(isDev ? [ "q:type", "A" ] : EMPTY_ARRAY), enqueue(ssr.closeFragment), 
            ...
            if ("string" == typeof type) {
                ...
                if (type === Fragment) {
                    ...
                    isDev && (attrs = [ "q:type", "F", ...attrs ]), ssr.openFragment(attrs), enqueue(ssr.closeFragment);
                    const children = jsx.children;
                    null != children && enqueue(children);
                } else if (type === Slot) {
                    ...
                    if (componentFrame) {
                        ...
                        ssr.openFragment(isDev ? [ "q:type", "P" ] : EMPTY_ARRAY), ssr.closeFragment();
                    }
                    ...
                    ssr.openComponent(isDev ? [ "q:type", "C" ] : []);
                    const host = ssr.getOrCreateLastNode();
          ...
          case 25:
            {
                ...
                signal.$hostElement$ = d[4], signal.$effects$ = new Set(d.slice(5)), inflateWrappedSignalValue(signal);
                break;
            }
...
function _getQContainerElement(element) {
    return (vnode_isVNode(element) ? vnode_getDomParent(element, !0) : element).closest(QContainerSelector);
}
...
const useId = () => {
    ...
    let id = `${base}${componentQrl?.getHash().substring(0, 3) || ""}${getNextUniqueIndex(iCtx.$container$) || ""}`;
